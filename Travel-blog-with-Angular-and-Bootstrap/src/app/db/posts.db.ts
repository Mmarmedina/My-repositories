import { Post } from "../interfaces/post.interface";

export const POSTS: Post [] =  [
    {
      id: 1, 
      title: 'Descubriendo París',
      text: 'Un viaje inolvidable por la Ciudad de la Luz. París, la capital de Francia, es una ciudad icónica llena de historia, arte y romance. Desde la majestuosidad de la Torre Eiffel hasta la serenidad de los Jardines de Luxemburgo, cada rincón de París ofrece una experiencia única para los viajeros. Pasear por las calles empedradas del Barrio Latino o explorar el Louvre, hogar de invaluables obras maestras, son solo algunas de las muchas actividades que te esperan en esta ciudad fascinante.',
      excerpt: 'Un viaje inolvidable por la Ciudad de la Luz.',
      author: 'Viajero Aventurero',
      img: '/assets/img/pexels-alfonso-escalante-2533092.jpg',
      date: '01-01-2024',
      id_category: 1
    },
    {
      id: 2,
      title: 'Explorando Machu Picchu',
      text: 'Una experiencia única en las alturas de los Andes. Machu Picchu, la antigua ciudad inca, es uno de los destinos más impresionantes del mundo. Subir por el Camino Inca y llegar a este sitio arqueológico es un logro emocionante. Las vistas panorámicas de las montañas y la mística atmósfera de Machu Picchu te transportan a otra época. En este viaje, descubrirás la rica historia de los incas y la belleza natural que rodea este sitio excepcional.',
      excerpt: 'Una experiencia única en las alturas de los Andes.',
      author: 'Explorador Intrépido',
      img: '/assets/img/pexels-alfonso-escalante-2533092.jpg',
      date: '01-11-2024',
      id_category: 2
    },
    {
      id: 3,
      title: 'Playas paradisíacas en Tailandia',
      text: 'Relajándose en las hermosas playas tailandesas. Tailandia es conocida por sus playas de arena blanca y aguas cristalinas. En este viaje, explorarás algunas de las islas más impresionantes del país, como Phuket y Koh Phi Phi. Sumérgete en la cultura tailandesa, disfruta de la deliciosa cocina local y relájate bajo el cálido sol tropical. Descubre por qué Tailandia es un destino de ensueño para los amantes de la playa y los aventureros por igual.',
      excerpt: 'Una experiencia única en las alturas de los Andes.',
      author: 'Amante del Sol',
      img: '/assets/img/pexels-alfonso-escalante-2533092.jpg',
      date: '01-05-2024',
      id_category: 3
    },
    {
      id: 4,
      title: 'Aventuras en la Patagonia',
      text: 'Explorando la belleza salvaje de la Patagonia. La Patagonia, con sus vastos paisajes y naturaleza virgen, es el sueño de cualquier amante de la aventura. Desde el Parque Nacional Torres del Paine hasta los glaciares imponentes, este rincón del mundo te dejará sin aliento. En este relato, compartiré mis experiencias al caminar por senderos remotos, acampar bajo un cielo estrellado y maravillarme con la diversidad de la fauna patagónica. Únete a mí en esta emocionante aventura en el extremo sur de Sudamérica.',
      excerpt: 'Una experiencia única en las alturas de los Andes.',
      author: 'Nómada Patagónico',
      img: '/assets/img/pexels-alfonso-escalante-2533092.jpg',
      date: '11-03-2024',
      id_category: 2
    },
    {
      id: 5,
      title: 'Cultura en Kioto',
      text: 'Sumergiéndose en la rica historia de Kioto, Japón. Kioto, la antigua capital de Japón, es un tesoro de cultura e historia. Desde los impresionantes templos hasta los jardines zen, cada rincón de Kioto cuenta una historia fascinante. En este viaje, exploraré los rincones menos conocidos de la ciudad, disfrutaré de la ceremonia del té y me perderé en los callejones tradicionales. Acompáñame en este viaje para descubrir la esencia de la cultura japonesa en Kioto.',
      excerpt: 'Una experiencia única en las alturas de los Andes.',
      author: 'Historiador Viajero',
      img: '/assets/img/pexels-alfonso-escalante-2533092.jpg',
      date: '15-06-2024',
      id_category: 1
    },
    {
      id: 6,
      title: 'Aventura en el Gran Cañón',
      text: 'Explorando los vastos cañones y paisajes impresionantes del Gran Cañón en Arizona. Este viaje me llevó a través de senderos desafiantes y miradores espectaculares que ofrecen vistas panorámicas de las capas geológicas únicas del cañón. Acampar bajo las estrellas en el borde del cañón fue una experiencia inolvidable, conectándome con la naturaleza en su forma más grandiosa.',
      excerpt: 'Una experiencia única en las alturas de los Andes.',
      author: 'Aventurero del Gran Cañón',
      img: '/assets/img/pexels-alfonso-escalante-2533092.jpg',
      date: '15-08-2024',
      id_category: 2
    },
    {
      id: 7,
      title: 'Exploración en la Selva Amazónica',
      text: 'Adentrándome en la densa selva amazónica, una de las maravillas naturales más sorprendentes de la Tierra. Este viaje me permitió descubrir la asombrosa diversidad de flora y fauna, desde coloridas aves exóticas hasta majestuosos jaguares. Navegar por los ríos serpenteantes y aprender de las comunidades indígenas locales fue una experiencia enriquecedora e inolvidable.',
      excerpt: 'Una experiencia única en las alturas de los Andes.',
      author: 'Explorador de la Amazonía',
      img: '/assets/img/pexels-alfonso-escalante-2533092.jpg',
      date: '14-06-2024',
      id_category: 4
    },
    {
      id: 8,
      title: 'Explorando la Campiña Inglesa',
      text: 'Un tranquilo viaje a través de los pintorescos paisajes rurales de la campiña inglesa. Descubre encantadores pueblos, extensos campos verdes y antiguas casas de campo. Disfruta de la serenidad del entorno y conéctate con la auténtica vida rural.',
      excerpt: 'Una experiencia única en las alturas de los Andes.',
      author: 'Amante de la Naturaleza',
      img: '/assets/img/pexels-alfonso-escalante-2533092.jpg',
      date: '15-02-2024',
      id_category: 4
    },
    {
      id: 9,
      title: 'Aventura en las Tierras Altas de Escocia',
      text: 'Exploración de las tierras altas escocesas, donde las montañas escarpadas y los lagos pintorescos te sumergen en la belleza natural. Recorre senderos rurales, visita castillos históricos y disfruta de la hospitalidad de las comunidades locales.',
      excerpt: 'Una experiencia única en las alturas de los Andes.',
      author: 'Explorador Rural',
      img: '/assets/img/pexels-alfonso-escalante-2533092.jpg',
      date: '20-06-2024',
      id_category: 4
    },
    {
      id: 10,
      title: 'Vibrante Festival de Música en Coachella',
      text: 'Sumérgete en la energía vibrante del famoso Festival de Música de Coachella. Descubre actuaciones emocionantes, arte visual impresionante y la diversidad de la cultura festivalera. Un viaje que te llevará a través de la música y la creatividad en el desierto de California.',
      excerpt: 'Una experiencia única en las alturas de los Andes.',
      author: 'Melómano Viajero',
      img: '/assets/img/pexels-alfonso-escalante-2533092.jpg',
      date: '15-06-2025',
      id_category: 5
    },
    {
      id: 11,
      title: 'Celebración del Carnaval de Río de Janeiro',
      text: 'Sumérgete en la colorida y animada atmósfera del Carnaval de Río de Janeiro. Vive la magia de los desfiles, los trajes espectaculares y la música contagiosa mientras exploras la rica cultura de Brasil durante esta festividad icónica.',
      excerpt: 'Una experiencia única en las alturas de los Andes.',
      author: 'Explorador Festivo',
      img: '/assets/img/pexels-alfonso-escalante-2533092.jpg',
      date: '15-07-2024',
      id_category: 5
    }
]